...
void Riscv::handleSupervisorTrap()
{
    ...
    if (scause == 0x0000000000000008UL || scause == 0x0000000000000009UL)
    {
        ...
        switch (opcode) {
            ...
            case SEM_OPEN: {
                SysSem* sysSem = SysSem::openSemaphore((int) arg2);
                *((SysSem**)(arg1)) = sysSem;
                ret = (uint64)sysSem;
                break;
            }
            case SEM_CLOSE: {
                SysSem * sysSem = (SysSem*)arg1;
                ret = SysSem::closeSemaphore(sysSem);
                break;
            }
            case SEM_WAIT: {
                SysSem * sysSem = (SysSem*)arg1;
                ret = SysSem::waitSemaphore(sysSem);
                break;
            }
            case SEM_SIGNAL: {
                SysSem * sysSem = (SysSem*)arg1;
                ret = SysSem::signalSemaphore(sysSem);
                break;
            }
            case SEM_TRYWAIT: {
                SysSem * sysSem = (SysSem*)arg1;
                ret = SysSem::trywaitSemaphore(sysSem);
                break;
            }
            case GETC: {
                ret = __getc();
